...
router.get('/:id', async (req, res) => {
  const item = await db.collection('users').findById(req.params.id)
...
router.post('/', async (req, res) => {
  ...
  const item = await db.collection('users').findById(id)
  res.status(201).json(mapDoc(item))
})

router.put('/:id', async (req, res) => {
  const ok = await db.collection('users').updateOne(req.params.id, req.body)
